...
const Pricing = () => {
  const features = [
    "Создание интеллектуальных ботов, улучшающих взаимодействие с клиентами",
    "Разработка сайтов с использованием ИИ для оптимизации пользовательского опыта",
    "Мастер-класс по продажам IT-продуктов, улучшающий ваши навыки ведения бизнеса",
    "Оффлайн встреча с лекторами для обмена знаниями и опытом",
    "Скидка на основное обучение, делающая обучение более доступным",
    "Лекция по карьерному развитию в IT, помогающая вам продвигаться по карьерной лестнице",
  ];
  const bonuses = [
    "Запись всех 3 дней, чтобы вы могли вернуться к материалам в любое время",
    "Эксклюзивный доступ в закрытый телеграм-канал для общения с единомышленниками",
    "Бесплатный доступ к курсу по HTML и CSS, улучшающему ваши навыки веб-разработки",
  ];
  return (
